...
with open('static\json\credentials.json', 'r') as file:
    data = json.load(file)
...
for key, value in data.items():
    ...
    append_dict = {key: hashed_password}
    DEFAULT_CREDENTIALS.update(append_dict) 
...
def load_pizzas():
    try:
        with open("static/json/pizza.json", "r") as f:
            return json.load(f)
...
def save_pizzas(pizzas):
    ...
    with open("static/json/pizza.json", "w") as f:
...
def login():
    if request.method == "POST":
        ...
        if username in DEFAULT_CREDENTIALS:
            stored_password = DEFAULT_CREDENTIALS[username]
        ...
        if user and check_password(user[2], password):  # user[2] is the hashed password in the database
            session['user'] = user[1]  # user[1] is the username
            return redirect(url_for('index'))
        else:
...
def password_reset():
    ...
    if request.method == "POST":
        ...
        conn = sqlite3.connect("users.db")
        c = conn.cursor()
        query = f"UPDATE users SET password = ? WHERE username = ?", (new_password, username,)
        c.execute(query)
        conn.commit()
        conn.close()
...
if __name__ == "__main__":
    ...
    if not os.path.exists("static/json/pizza.json"):
        save_pizzas([])  
